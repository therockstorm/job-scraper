...
const clientStub = sinon.stub(new ChallengeClient());
let scraper;
...
describe('scrapePage', () => {
  ...
  it('scrape jobs', () => {
    ...
    return scraper.scrapePage(FULL_HTML) // eslint-disable-line no-use-before-define
  ...
  it('return empty string if unable to parse', () => {
    ...
    return scraper.scrapePage(html)
      .then(res => assert.deepEqual(res, expected));
  });
});
